...
const FEEDBACK_ACTION_OPTIONS = [
  ...
  { value: "repaid", label: "Repaid", color: "peer-data-[state=checked]:bg-blue-50 peer-data-[state=checked]:text-blue-700 peer-data-[state=checked]:border-blue-500" },
...
function UnifiedFeedbackInterface() {
  const [activeTab, setActiveTab] = useState<"brova" | "final">("brova");
  ...
  const [isInvestigationNeeded, setIsInvestigationNeeded] = useState(false);
  ...
  useEffect(() => {
    if (activeOrder?.garments?.length) {
      ...
      setIsInvestigationNeeded(false);
  ...
  useEffect(() => {
    if (feedbackAction && feedbackAction !== "accepted") {
  ...
  const onConfirmClick = () => {
    if (!satisfaction || !feedbackAction || !distributionAction) {
        toast.error("Please complete all feedback sections");
        return;
    }
    if (activeTab === "brova" && !customerSignature) {
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        // Closing logic redirect
        if (feedbackAction === "accepted" || feedbackAction === "redo") {
             if (balance > 0) {
                 toast.info("Order has pending balance. Redirecting to Payment...");
                 // navigate({ to: `/$main/orders/${activeOrder?.id}/payment` }); 
             } else if (balance < 0) {
                 toast.info("Order has overpayment. Redirecting to Refund...");
                 // navigate({ to: `/$main/orders/${activeOrder?.id}/refund` }); 
             } else {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-4">
            <div>
                <h1 className="text-3xl font-bold text-foreground">
                    <span className="capitalize">{activeTab}</span> <span className="text-primary">Feedback</span>
                </h1>
            ...
            >
                <TabsList className="bg-transparent h-10">
                    <TabsTrigger value="brova" className="px-8 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm cursor-pointer">Brova</TabsTrigger>
                    <TabsTrigger value="final" className="px-8 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm cursor-pointer">Final</TabsTrigger>
                </TabsList>
            </Tabs>
        </div>
      ...
      {activeOrder ? (
        ...
        >
                        ...
                        <Table>
                            <TableHeader className="bg-muted/50 sticky top-0 z-10 border-b-2 border-border/60">
                                <TableRow className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                                    ...
                                    <TableHead className="text-center w-[12%] bg-primary/5 p-3">{activeTab === 'brova' ? 'Brova' : 'Final'} (cm)</TableHead>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* CUSTOMER SENTIMENTS */}
              <Card className="border-2 border-border shadow-lg rounded-3xl overflow-hidden flex flex-col">
                  <CardHeader className="bg-muted/30 border-b p-6">
                      <div className="flex items-center gap-4">
                          ...
                          <div>
                              <CardTitle className="text-xl font-black uppercase tracking-tight">Customer Sentiments</CardTitle>
                              <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">Record customer satisfaction and signature</p>
                          </div>
                      </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-8 flex-1">
                      ...
                      <div className="space-y-4">
                          ...
                          >
                              {SATISFACTION_LEVELS.map((level) => (
                                  <div key={level.value} className="flex-1 min-w-[80px]">
                      ...
                      {activeTab === "brova" && (
                          <div className="space-y-4 pt-4 border-t border-border/60">
                              <div className="flex items-center justify-between">
                                  <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Customer Signature*</Label>
                                  {customerSignature && <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 font-black text-[8px]">SIGNED</Badge>}
                              </div>
                              <Dialog>
                                  <DialogTrigger asChild>
                                      <Button 
                                          variant="outline" 
                                          className={cn(
                                              "w-full h-24 border-2 border-dashed flex flex-col gap-2 bg-muted/5 transition-all",
                                              customerSignature ? "border-emerald-500/50 bg-emerald-50/30" : "hover:border-primary/50 hover:bg-primary/5"
                                          )}
                                      >
                                          {customerSignature ? (
                                              <img src={customerSignature} alt="Signature" className="h-20 object-contain" />
                                          ) : (
                                              <>
                                                  <PenTool className="w-6 h-6 text-muted-foreground/40" />
                                                  <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Open Signature Pad</span>
                                              </>
                                          )}
                                      </Button>
                                  </DialogTrigger>
              ...
              {/* ORDER ACTIONS */}
              <Card className="border-2 border-primary/20 shadow-lg rounded-3xl overflow-hidden flex flex-col">
                  <CardHeader className="bg-primary/5 border-b p-6">
                      <div className="flex items-center gap-4">
                          ...
                          <div>
                              <CardTitle className="text-xl font-black uppercase tracking-tight">Order Action</CardTitle>
                              <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">Define next steps and status</p>
                          </div>
                      </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6 flex-1">
                      <div className="space-y-4">
                          <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Status Action</Label>
                          <RadioGroup 
                              value={feedbackAction || ""} 
                              onValueChange={setFeedbackAction}
                      ...
                      {/* Investigation Option (Visible only for Brova + Redo) */}
                      {activeTab === "brova" && feedbackAction === "redo" && (
                          <motion.div 
                              initial={{ opacity: 0, height: 0 }}
                              animate={{ opacity: 1, height: "auto" }}
                              className="p-4 bg-red-50 rounded-2xl border-2 border-red-200 space-y-3"
                          >
                              <div className="flex items-center gap-3">
                                  <AlertCircle className="w-5 h-5 text-red-600" />
                                  <div className="flex-1">
                                      <p className="text-xs font-black text-red-700 uppercase tracking-tight">Investigation Required?</p>
                                      <p className="text-[10px] font-medium text-red-600">Should this redo trigger a quality investigation?</p>
                                  </div>
                                  <Checkbox 
                                      id="investigation" 
                                      checked={isInvestigationNeeded}
                                      onCheckedChange={(c) => setIsInvestigationNeeded(c as boolean)}
                      ...
                      <div className="space-y-4">
                          <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Distribution Flow</Label>
                          <RadioGroup 
                          ...
                          >
                              {ORDER_DISTRIBUTION_OPTIONS.map((opt) => {
                                  const isDisabled = !!(feedbackAction && feedbackAction !== "accepted" && opt.value !== "workshop");
                                  return (
                                      <div key={opt.value}>
                                          <RadioGroupItem 
                                              value={opt.value} 
                                          ...
                                          <Label
                                              ...
                                              className={cn(
                                                  "flex items-center gap-3 h-12 rounded-xl border-2 border-border bg-card px-4 cursor-pointer transition-all shadow-sm peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5",
                                                  isDisabled && "opacity-40 cursor-not-allowed grayscale"
                                              )}
                                          >
                                              <opt.icon className="w-3.5 h-3.5 text-muted-foreground" />
                                              <span className="font-black uppercase tracking-widest text-[9px] flex-1">{opt.label}</span>
                                          </Label>
                                      </div>
                                  );
                              })}
                      ...
                      <div className="pt-2">
                          <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 block mb-2">Finalization Notes</Label>
                          <Textarea 
                              placeholder="Add any final notes about this feedback session..."
                      ...
                      <Button 
                          ...
                          disabled={!satisfaction || !feedbackAction || !distributionAction || isSubmitting}
                          className="w-full h-14 font-black uppercase tracking-widest shadow-xl shadow-primary/20 text-base rounded-2xl"
                      >
          ...
          <ConfirmationDialog
              ...
              description={`You are about to finalize feedback for Order #${activeOrder.id}. This action will proceed to close the order logic.`}
              confirmText="Proceed"
              cancelText="Go Back"
          />
        ...
        >
            ...
            <h3 className="text-2xl font-black text-foreground uppercase tracking-tight">Select an Order</h3>
            <p className="text-muted-foreground font-medium uppercase tracking-widest text-[10px] mt-2">Enter an ID or search for a customer to begin {activeTab} feedback</p>
        </motion.div>
      )}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-bold opacity-80 uppercase tracking-widest">
                            Choose an active order for this customer
                          </p>
                        </div>
                      </div>
                    </div>
                          ...
                          >
                            <div className="flex items-center gap-6">
                              ...
                              <div className="flex flex-col gap-1.5">
                                ...
                                <div className="flex items-center gap-2 text-xs font-bold text-foreground">
                                   ...
                                   {order.production_stage?.replace(/_/g, " ") ?? "IN PROGRESS"}
                                </div>
                              </div>
                            </div>
                            <div className="flex flex-col items-end">
                                ...
                                <div className="flex items-center gap-2 text-primary font-black text-xs">
                                   ...
                                   {order.delivery_date ? format(new Date(order.delivery_date), "MMM d") : "N/A"}
                                </div>
                            </div>
                          </button>
                        ))}
