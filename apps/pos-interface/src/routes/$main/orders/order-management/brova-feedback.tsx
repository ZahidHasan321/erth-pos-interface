...
const FEEDBACK_STATUS_OPTIONS = [
  { value: "perfect", label: "Perfect Fit", color: "text-green-600" },
  { value: "minor-adj", label: "Minor Adjustments", color: "text-yellow-600" },
  { value: "major-adj", label: "Major Adjustments", color: "text-orange-600" },
  { value: "redo", label: "Full Redo Required", color: "text-red-600" },
];
...
function BrovaFeedbackInterface() {
  ...
  const [feedbackStatus, setFeedbackStatus] = useState<string>("perfect");
  ...
  const onConfirmClick = () => {
    if (!receivingAction) {
        toast.error("Please select an action (Proceed or Return)");
        return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
        toast.success(`Feedback Logged`, {
            description: `Garment ${activeGarment?.garment_id} processed with status: ${FEEDBACK_STATUS_OPTIONS.find(o => o.value === feedbackStatus)?.label}`
        });
        setActiveOrder(null);
    } catch (err) {
          ...
          <Tabs value={selectedGarmentId || ""} onValueChange={setSelectedGarmentId} className="w-full space-y-6">
            ...
            <TabsContent value={selectedGarmentId || ""} className="mt-0 space-y-8 focus-visible:ring-0">
                        ...
                        <Table>
                            <TableHeader className="bg-muted/50 sticky top-0 z-10 border-b-2 border-border/60">
                                <TableRow className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                                    ...
                                    <TableHead className="text-center w-[15%] bg-primary/5">Adjusted (cm)</TableHead>
                ...
                <Card className="border-2 border-primary shadow-xl shadow-primary/5 rounded-3xl overflow-hidden">
                    <CardHeader className="bg-primary/5 border-b-2 border-primary/10 p-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] mt-1">Submit brova results to proceed with production</p>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="p-8 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            ...
                            <div className="space-y-4">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Classification of Fit</Label>
                            ...
                            {/* Receiving Action */}
                            <div className="space-y-4">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Production Decision</Label>
                                <RadioGroup 
                                ...
                                >
                                    <div>
                                        ...
                                        >
                                            ...
                                            <span className="font-black uppercase tracking-widest text-[10px]">Approve & Produce</span>
                                        </Label>
                                    </div>
                                    <div>
                                        ...
                                        >
                                            ...
                                            <span className="font-black uppercase tracking-widest text-[10px]">Alteration Required</span>
                                        </Label>
                                    </div>
                                </RadioGroup>
                            </div>
                ...
                <ConfirmationDialog
                    ...
                    title={receivingAction === 'accept' ? "Confirm Brova Approval" : "Confirm Alteration Request"}
                    description={receivingAction === 'accept' 
                        ? `You are approving garment ${activeGarment?.garment_id}. This will trigger final production.` 
                        : `You are flagging garment ${activeGarment?.garment_id} for alterations. It will be returned to the workshop.`
                    }
                    confirmText={receivingAction === 'accept' ? "Yes, Approve" : "Yes, Request Alterations"}
                    cancelText="Go Back"
                />
