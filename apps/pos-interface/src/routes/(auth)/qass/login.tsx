...
export const Route = createFileRoute("/(auth)/qass/login")({
  validateSearch: z.object({
    redirect: z.string().optional().catch(""),
  }),
  beforeLoad: ({ context, search }) => {
    if (context.auth.isAuthenticated) {
      throw redirect({ to: search.redirect || "/qass" });
    }
  },
  component: QassLoginPage,
  head: () => ({ meta: [{ title: "Qass: Sign In" }] }),
});
...
function QassLoginPage() {
  ...
  // Apply qass theme so CSS vars resolve correctly
  React.useEffect(() => {
    document.documentElement.classList.remove("erth", "sakkba", "qass");
    document.documentElement.classList.add("qass");
    return () => document.documentElement.classList.remove("qass");
  }, []);
        ...
        setUsers(
          (data as ShopUser[]).filter(
            (u) =>
              ...
              (!u.brands || u.brands.length === 0 || u.brands.includes("qass"))
          )
        );
      }
    });
  ...
  React.useEffect(() => {
    if (auth.isAuthenticated) {
      router.invalidate().then(() => navigate({ to: search.redirect || "/qass" }));
    }
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 40 }}>
                <img
                  src={QassLogo}
                  alt="Qass"
                  style={{ height: 16, width: "auto", filter: "brightness(0) invert(1) opacity(0.85)" }}
                />
              ...
              <p style={{
                ...
                Bespoke fittings &<br />alterations, managed.
              </p>
            </div>
