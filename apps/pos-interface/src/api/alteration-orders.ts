...
export const getCustomerGarmentsForLink = async (customerId: number) => {
    const { data, error } = await db
        .from("garments")
        .select("id, garment_id, garment_type, alteration_measurements, alteration_styles, full_measurement_set_id, measurement_id, measurement:measurements!measurement_id(*), collar_type, collar_button, collar_thickness, cuffs_type, cuffs_thickness, front_pocket_type, front_pocket_thickness, wallet_pocket, pen_holder, mobile_pocket, small_tabaggi, jabzour_1, jabzour_2, jabzour_thickness, lines, order_id, orders!inner(customer_id, order_date)")
