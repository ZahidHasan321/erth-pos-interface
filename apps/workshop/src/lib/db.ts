 ...
 *
 * Kept structurally in sync with apps/pos-interface/src/lib/db.ts — the
 * transient-retry / idempotency policy must be identical in both apps.
 */
...
// with a TypeError ("NetworkError when attempting to fetch resource").
//
...
// IDEMPOTENT methods (GET/HEAD). Non-idempotent writes are NOT retried here —
// those get explicit, idempotency-keyed retry at the API layer. We also never
// replay timeouts (server may have committed) or caller cancellations.
...
async function timeoutFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      // (connection failure) AND only for idempotent methods.
