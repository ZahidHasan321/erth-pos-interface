...
export async function getUnreadCount(): Promise<number> {
  const { data, error } = await db.rpc("get_unread_notification_count", { p_department: "workshop" });
  if (error) throw error;
...
export async function markAllNotificationsRead(): Promise<void> {
  const { error } = await db.rpc("mark_all_notifications_read", { p_department: "workshop" });
  if (error) throw error;
}
