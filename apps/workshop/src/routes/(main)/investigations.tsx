...
import { PageHeader, SectionCard, EmptyState, StatusBanner } from "@/components/shared/PageShell";
import { useAuth } from "@/context/auth";
import { isAdmin, isManager } from "@/lib/rbac";
import { ROOT_CAUSES } from "@/lib/root-causes";
import {
  getNeedsInvestigation, recordInvestigation, qualityReturns, alterationReturns,
...
export const Route = createFileRoute("/(main)/investigations")({
  ...
  head: () => ({ meta: [{ title: "Needs Investigation" }] }),
});
...
function InvestigationsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      <PageHeader icon={ShieldAlert} title="Needs Investigation" />

      {!canResolve && (
        <StatusBanner tone="info">
          Garments held after repeated returns. Only a manager can record the investigation and release the hold.
        </StatusBanner>
      )}
      ...
      {isLoading ? (
        ...
        <EmptyState icon={ShieldAlert} message="No garments need investigation" />
      ) : (
        <SectionCard
          title={`${garments.length} held garment${garments.length === 1 ? "" : "s"}`}
        >
