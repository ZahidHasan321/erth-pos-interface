...
export const Route = createFileRoute("/(main)/store/active-deliveries")({
  component: ActiveDeliveriesPage,
  head: () => ({ meta: [{ title: "Active Deliveries" }] }),
});
...
function daysSince(dateStr: string | Date | null | undefined) {
  if (!dateStr) return 0;
  return Math.floor((Date.now() - parseUtcTimestamp(dateStr).getTime()) / (1000 * 60 * 60 * 24));
}
...
function getApprovalSummary(items: TransferRequestWithItems["items"]) {
  ...
  return { totalRequested, totalApproved, isPartial: totalApproved < totalRequested };
}
...
function AgeBadge({ dateStr }: { dateStr: string | Date | null | undefined }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-semibold", days >= 5 ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700")}>
...
function ActiveDeliveryRow({
  request, isExpanded, onToggle, onCancel,
}: {
  request: TransferRequestWithItems; isExpanded: boolean; onToggle: () => void; onCancel: () => void;
}) {
  ...
  return (
    <>
      <TableRow className="cursor-pointer" onClick={onToggle}>
        ...
        <TableCell className="py-3.5"><TransferStatusBadge status={request.status} /></TableCell>
        <TableCell className="py-3.5"><ItemTypeBadge itemType={request.item_type} /></TableCell>
        <TableCell className="py-3.5">
          <div>
          ...
          {(() => {
            ...
            return (
              <span className={cn("text-[10px] tabular-nums font-medium", summary.isPartial ? "text-amber-600" : "text-emerald-600")}>
        ...
        <TableCell className="py-3.5 text-right">
          {canCancel && (
            <Button size="sm" variant="outline" className="text-destructive border-destructive/30 hover:bg-destructive/5 hover:text-destructive" onClick={(e) => { e.stopPropagation(); onCancel(); }}>
      ...
      <TableRow className="border-0 hover:bg-transparent">
        <TableCell colSpan={8} className={cn("p-0 transition-colors", isExpanded ? "bg-muted/30 border-b border-border/40" : "border-0")}>
                    ...
                    {request.items.map((item) => (
                      <tr key={item.id} className="border-t border-border/50">
                        ...
                        <td className="py-2 text-right tabular-nums pr-2">
                          {item.approved_qty != null ? <span className="font-medium text-emerald-600">{item.approved_qty}</span> : "—"}
                        </td>
                        <td className="py-2 text-right tabular-nums pr-2">
                          {item.dispatched_qty != null ? <span className="font-medium">{item.dispatched_qty}</span> : "—"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
function ActiveDeliveriesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return requests.filter((r) => String(r.id).includes(q) || r.items.some((i) => getItemName(i).toLowerCase().includes(q)));
  }, [requests, search]);
    ...
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto space-y-5">
      <div className="flex items-start gap-3">
        <Button variant="outline" size="icon" asChild>
          <Link to="/store/request-delivery"><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        <div>
          <h1 className="text-xl font-bold tracking-tight">Active Deliveries</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {requests.length > 0
              ? `${requests.length} delivery request${requests.length === 1 ? "" : "s"} in flight`
              : "Requests pending approval or dispatch from the shop"}
          </p>
        </div>
      </div>
      ...
      {isLoading && requests.length === 0 ? (
        ...
        <Card className="shadow-none rounded-xl border">
          <CardContent className="py-14 text-center text-muted-foreground">
            ...
            <p className="text-xs mt-1 opacity-70">Requests you submit will appear here until received</p>
          </CardContent>
        </Card>
      ) : filtered.length === 0 ? (
        <Card className="shadow-none rounded-xl border">
      ...
      <Dialog open={!!confirmCancel} onOpenChange={(open) => !open && setConfirmCancel(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>This removes the request permanently. You can create a new request afterwards.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmCancel(null)} disabled={cancelTransfer.isPending}>Keep request</Button>
            <Button variant="destructive" onClick={handleCancel} disabled={cancelTransfer.isPending}>
              {cancelTransfer.isPending && <Loader2 className="h-4 w-4 animate-spin mr-1.5" />}
              Cancel request
            </Button>
          </DialogFooter>
        </DialogContent>
