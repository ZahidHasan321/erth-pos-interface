                      ...
                      return (
                        ...
                        >
                          <div className="min-w-0 flex-1">
                            ...
                            <p className={`text-[11px] mt-0.5 ${outOfStock ? "text-[var(--status-bad)]" : "text-muted-foreground"}`}>
                        ...
                        <div key={`${line.itemType}:${line.itemId}`} className="px-3 py-2.5 space-y-2">
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <button
                                        ...
                                        className={`shrink-0 rounded-full ${mode === "send" ? "text-[var(--status-bad)]" : "text-[var(--status-warn)]"} hover:opacity-80 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring`}
                                      >
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <Input
                              ...
                              className={`w-[64px] h-7 text-center px-1 ${overStock && mode === "send" ? "border-[var(--status-bad)] focus-visible:ring-[color:var(--status-bad)]/30" : ""}`}
                            />
                            <Button
                              type="button"
                              variant="outline"
...
function DirectionTile({ active, from, to, onClick }: { active: boolean; from: string; to: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`text-left rounded-md border p-3 transition-all ${active ? "border-primary bg-primary/5 ring-1 ring-primary/30" : "hover:border-foreground/30 hover:bg-muted/40"}`}
    >
      <div className="flex items-center gap-2 text-sm font-medium">
        <span>{from}</span>
