...
export function TransferDetailDrawer({ open, onClose, transfer }: Props) {
  ...
  function handleReceive() {
    ...
    receiveMut.mutate({ transferId: transfer!.id, items: payload }, {
      onSuccess: (data: { success: boolean; transfer_id: number; has_discrepancy: boolean }) => {
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <div key={step.key} className="flex items-center gap-1.5 flex-1">
                  <div className={`h-5 w-5 rounded-full flex items-center justify-center text-[11px] font-medium shrink-0 ${dotClass}`}>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {transfer.notes && (
            <div className="rounded-md border bg-amber-50/40 border-amber-100 px-3 py-2 text-sm">
              <p className="text-[11px] uppercase tracking-wide text-amber-700 font-medium">Note</p>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium">Items ({transfer.items.length})</h3>
              {action === "dispatch" && (
                ...
                return (
                  <div key={it.id} className="border rounded-lg p-3 space-y-2.5">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {isReceived && <span className="text-[11px] uppercase font-medium text-green-700 bg-green-100 px-1.5 py-0.5 rounded shrink-0">received</span>}
                    </div>
...
function QtyCell({ label, value, highlight }: { label: string; value: number | string; highlight?: boolean }) {
  return (
    <div>
      <p className="text-[11px] uppercase text-muted-foreground tracking-wide">{label}</p>
      <p className={`font-medium tabular-nums ${highlight ? "text-blue-700" : ""}`}>{value}</p>
    </div>
  );
}
...
function QtyInput({ label, max, value, onChange, srcStock }: { label: string; max: number; value: string; onChange: (v: string) => void; srcStock?: number | null }) {
  ...
  return (
    <div>
      ...
      {tooHigh && <p className="text-[11px] text-red-600 mt-0.5 ml-[98px]">Exceeds maximum</p>}
      {!tooHigh && overStock && <p className="text-[11px] text-amber-600 mt-0.5 ml-[98px]">Exceeds available stock</p>}
    </div>
  );
}
