      ...
      <DialogContent className="max-w-lg p-0 gap-0 overflow-hidden">
        <DialogHeader className="px-6 pt-5 pb-4 border-b">
          <DialogTitle className="flex items-center gap-2 text-base min-w-0">
            <AlertTriangle className="h-4 w-4 shrink-0 text-[var(--status-bad)]" aria-hidden="true" />
            <span className="truncate">Damage / waste: {itemName}</span>
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-5 space-y-6">
            <Field label="Quantity wasted" hint={`On hand: ${formatQty(itemType, currentStock, unit)}`}>
              <div className="flex items-stretch rounded-md border border-input overflow-hidden focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-1">
                <button
                  type="button"
                  className="px-3 hover:bg-muted text-muted-foreground border-r disabled:opacity-40 disabled:cursor-not-allowed"
                  onClick={() => bump(-1)}
                  disabled={qty <= 0}
                ...
                <div className="flex-1 relative">
                  <Input
                    ...
                    className="h-12 border-0 rounded-none text-center text-xl font-semibold tabular-nums text-[var(--status-bad)] focus-visible:ring-0 pr-12"
                    placeholder="0"
                ...
                <button
                  type="button"
                  className="px-3 text-[var(--status-bad)] border-l hover:bg-[var(--status-bad-bg)] disabled:opacity-40 disabled:cursor-not-allowed"
                  onClick={() => bump(1)}
              ...
              {qty > 0 && (
                <div className="flex items-center justify-between gap-2 mt-2.5 text-sm tabular-nums rounded-md bg-muted/40 px-3 py-2">
                  <span className="text-muted-foreground">
                    {formatQty(itemType, currentStock, unit)} → <span className="font-medium text-foreground">{formatQty(itemType, newQty, unit)}</span>
                  </span>
                  {Number(unitCost ?? 0) > 0 && (
                    <span className={cn("font-medium", overThreshold ? "text-[var(--status-bad)]" : "text-muted-foreground")}>
                      Cost ≈ {cost.toFixed(2)}
                    </span>
                  )}
                </div>
            ...
            {blockedByApproval && (
              <div
                className="flex items-start gap-2.5 rounded-md border p-3 text-sm bg-[var(--status-warn-bg)] text-[var(--status-warn)] border-[color:var(--status-warn)]/30"
                role="status"
                aria-live="polite"
              >
                <ShieldAlert className="h-4 w-4 mt-0.5 shrink-0" aria-hidden="true" />
                <span>
                  This waste exceeds the {WASTE_APPROVAL_THRESHOLD} cost threshold, so a manager needs to record it. Ask a manager to confirm, or reduce the quantity.
                </span>
              </div>
            )}
          </div>
          ...
          <DialogFooter className="px-6 py-4 border-t bg-muted/30 gap-2">
            <Button type="button" variant="outline" onClick={onClose} disabled={mut.isPending}>Cancel</Button>
            <Button
              type="submit"
              variant="destructive"
              disabled={mut.isPending || qty <= 0 || blockedByApproval}
            >
              {mut.isPending && <Loader2 className="h-4 w-4 animate-spin mr-1.5" aria-hidden="true" />}
