...
- **Home-based brands — SAKKBA, QASS.** No showroom, no cashier — payment is taken **inline at order-taking** (§3). They run the **same brova + finals lifecycle** as the showroom (brova is offered at order-taking and, when chosen, tried at the shop per §2.5) — they differ in fulfilment, not in production: **delivery-only** (pickup removed; `home_delivery` forced true at order-taking), so the **finals** are handed over on a per-brand **Delivery page** — the home-brand analogue of the cashier's handover (§5) — rather than collected at a counter. They hold **no stock of their own**: their fabric is drawn from ERTH's shop stock.
...
**Home-based brand variant (§1).** A home-based brand's WORK order runs the **same** production spine and the **same brova trial**, differing only in payment and final handover. At **1B** garments are created `brova` or `final` exactly as the showroom does (brova-parking of finals applies). At **1C** payment is taken inline (no cashier queue; step 2 is skipped) and `home_delivery` is forced true (pickup is not offered). Production (3–6) and the **brova trial (8, §2.5)** are identical — the brova comes back to the shop and is **tried at the shop**, and any one accepted brova releases the finals. The difference is only the final **handover**: instead of a counter collection, once **every** garment of the order is deliverable at the shop — each final `ready_for_pickup` and each brova `brova_trialed` with `acceptance_status` true (an untried `awaiting_trial` brova, or one rejected back to the workshop, is **not** deliverable) — the whole order is handed over on the brand's **Delivery page** (§5), stamping the same handover on every garment including the accepted brova (`fulfillment_type: delivered`, `piece_stage: completed`, `order_phase: completed`). *Open gap:* a **brova-trial style change** (§2.5) re-prices the order, but a home-based brand has no cashier to settle the delta and the Delivery page takes no money — settlement of a home-brand trial-time reprice is unresolved.
...
- **Reversing an acceptance re-parks — or blocks on — the released finals.** Editing a brova's feedback from an accepting verdict (Accept / Accept-with-Fix) to a rejecting one (Reject-Repair or Reject-Redo), which the gate above still permits until finals start production, flips the brova's `acceptance_status` back to `false`. A brova acceptance is what *releases* the order's parked finals, so the reversal must not strand finals released under a now-rejected brova. On submit, when **no other brova in the order still accepts it** (`acceptance_status: true` or `completed` — the same "any brova accepted" gate that releases finals), each of the order's finals is reconciled: a final still parked (`waiting_for_acceptance`) is left as-is; a final **released but not yet scheduled** (`waiting_cut` with no `production_plan`) is **re-parked** to `waiting_for_acceptance` (clearing `in_production`/`production_plan`/`assigned_date`) so it waits for the next trial; a final already **scheduled or started** (`waiting_cut` carrying a `production_plan`, any `cutting`-or-later stage, or `in_production`) **blocks the whole reversal** with an error naming the final — staff reverse it at the workshop first. When another brova still accepts, the finals belong to that acceptance and stay released. Enforced **server-side, atomically** (`FOR UPDATE` on the finals) so a concurrent workshop release cannot slip a final past the check between the shop reading the order and submitting; before submit the shop confirms in a dialog listing the finals that will be re-parked.
...
**Shoulder slope** is recorded as one of ten fixed values, picked from a plain text dropdown (no shape glyphs) — **NORMAL**, **RIGHT SHOULDER DOWN**, **RIGHT SHOULDER UP**, **RIGHT SHOULDER STRAIGHT**, **LEFT SHOULDER DOWN**, **LEFT SHOULDER UP**, **LEFT SHOULDER STRAIGHT**, **LEFT AND RIGHT SHOULDER DOWN**, **LEFT AND RIGHT SHOULDER UP**, **LEFT AND RIGHT SHOULDER STRAIGHT**. **NORMAL** (stored enum value `none`) is an explicit stored value ("no notable slope") — distinct from an absent/never-filled value (null) and from the STRAIGHT values. On the shop new-measurements form a **new** measurement seeds NORMAL as the overwritable predicted default (so the common case needs no action); it is still required and overwritable, and a legacy row whose value is absent (null) reads as unfilled and must be answered on edit. The picker sits side by side with collar position in one row.
...
**Numeric field requiredness & QC scope (client measurement-naming spec).** Each numeric measurement is **required** or **optional** on the shop new-measurement form, and QC checks a slightly different set — the two surfaces are listed separately by the client and may disagree per field. The optional (blank-allowed) fields on the form are **Full Chest**, **Waist Full**, **2nd Button Distance**, **Pen Pocket Length/Width**, and the **Basma** pair (basma is conditional — entered only when the basma option is chosen — so it stays optional); every other numeric field is required. Three required fields seed a **predicted default that the operator can overwrite** when creating a new measurement: **Jabzour Width = 1.5**, **Sleeve Hemming = 4**, **Bottom Hemming = 4** (the seed is applied only on a fresh measurement, never surfaced on an empty/read-only view). For **QC**: **Full Chest and Waist Full are dropped entirely** (not measured at the workshop), **Bottom Hemming is optional** (required on the form but not verified at QC), and a blank cell on any QC-optional field (2nd Button Distance, Pen Pocket, Bottom Hemming, Basma) never blocks a QC pass; everything else QC checks is required. The on-screen order of both surfaces follows the client's listed sequence.
...
- **Payment flows through the cashier (like WORK).** A confirmed alteration is **pending cashier processing** and **cannot be dispatched** to the workshop until the cashier confirms-without-payment or takes payment (§3 gate; marker on `alteration_orders.cashier_processed_at`). This is the intake-payment gate; the collect-only handover above is the separate finished-garment step.
- **Per-garment reason matrix + printable form.** Intake also captures an optional **reason matrix** per garment (row = garment part e.g. MEASURE / COLLAR / HEMMING; column = CUSTOMER REQUEST CHANGE / GARMENT NOT SAME FATOURA / BAD QUALITY), persisted to `garments.alteration_issues`. A **printable alteration form PDF** (one page per garment: body diagram with the changed measurements, the marked reason matrix, and header/side metadata) is available after creating the order and from order history.

### 2.15 Undo window — take back a mistaken logistics move

A handful of high-regret garment-movement actions offer a short **Undo** window (a toast with an Undo button for ~60 seconds after the action). This is a **convenience**, not a general rollback: only the actions below are undoable, and only while it is still safe.

- **Undoable actions.** Workshop **dispatch → shop**; shop **dispatch → workshop** (first dispatch and return trips); **receive** on either side (workshop receive, shop receive-back); **schedule** a garment; **park → scheduler** (send-to-scheduler); and **advance a piece to the next stage**. No other action is undoable (payments, QC verdicts, feedback, stock moves, cancellations are not).
- **Safe-window gate (the correctness rule).** Undo restores the garment's exact pre-action state **only if nothing downstream has consumed the action's result** — specifically, the garment must still match, on `location` / `piece_stage` / `in_production` / `start_time` / `completion_time`, the state the action left it in. So a dispatch can be undone **only while the garment is still in transit** (the counterpart has not received it); a receive/schedule/advance only **while the next step has not started**. Once the other side receives, or a worker starts/finishes the next stage, the window is closed and undo is **refused** (the action stands). This gate is enforced server-side, so undo can never corrupt state even under a race — the toast is only the UX affordance.
- **Also expires by time and is single-use.** Undo is refused after the window lapses (~60s) and after it has been used once. Undoing a dispatch fully unwinds it: it **voids** the dispatch-history audit row (the dispatch reads as if it never happened, append-only preserved), **retracts** the "dispatched" notification it fired to the other side, and — when it was the order's *only* dispatch (no garment still out) — resets the order's phase back out of `in_progress`. Reverting a receive does **not** re-notify the other side.
...
- **WORK cashier-processing gate.** A confirmed WORK order is **pending cashier processing** until a cashier acts on it, and **cannot be dispatched to the workshop** before then. Processing is either (a) **confirm without payment** — clears the gate with no money and **no open register required** — or (b) recording **any payment** (advance/full/partial, which does require an open register). Either sets a one-time `cashier_processed_at` marker (idempotent; never cleared). The **marker, not payment, is the dispatch gate**: a confirmed-without-payment order (still `paid: 0`) is dispatchable; an unprocessed order is not (the `dispatch_order` RPC rejects it, and the dispatch queue hides it). The marker is also set whenever a WORK order's first payment is recorded anywhere (bulk page or per-order detail), so a paid order is never stranded as pending. **ALTERATION is gated exactly like WORK**: a confirmed alteration is pending cashier processing until confirmed-without-payment or paid, and cannot be dispatched before then — but because an alteration has no `work_orders` row, its marker lives on `alteration_orders.cashier_processed_at`. An alteration has no advance (Full-only in the bulk page) and no order-linking, and its finished-garment handover remains collect-only with no feedback form (§2.14). **Only SALES is not gated** and never enters the Pending queue. Order-taker confirm persists the order's agreed **advance** to `work_orders.advance` for ERTH too (informational — drives the cashier's Advance preset; it is NOT a payment, so `paid` stays 0).
...
**Home-based brand order-taking (§1).** For a home-based brand the new-work-order form takes payment **inline** (no cashier deferral) and **forces home delivery** — the Pick-Up option is not shown and `home_delivery` is set true on every garment. The **brova option is offered** just as it is for the showroom (the Brova column appears on the fabric form); a chosen brova runs the full §2.5 trial, tried at the shop, and the finals are handed over on the Delivery page (§5).
...
**Delivery page (home-based brands).** Each home-based brand shell carries a dedicated **Delivery** surface — the home-brand analogue of ERTH's cashier handover (§3), scoped to that brand and **never shown on ERTH**. It lists the brand's confirmed WORK orders that are **ready to deliver**: every garment of the order back at the shop and deliverable — each final `ready_for_pickup`, and each brova **accepted at the shop trial** (`brova_trialed` with `acceptance_status` true). A brova still `awaiting_trial` (not yet tried) or rejected back to the workshop is **not** deliverable and holds the whole order back — enforcing "the brova is tried at the shop before handover" (§1). Delivery is **whole-order, all-or-nothing** — the action is enabled only when **all** the order's garments are deliverable, and completing it hands over the entire order in one step through the shared handover RPC (every garment, including the accepted brova → `piece_stage: completed`, `fulfillment_type: delivered`; `order_phase: completed`). A delivered order leaves the ready list for a delivered/history view. **No money is handled here** (payment was taken at order-taking) — it is purely the final-handover completion these brands otherwise lack.
