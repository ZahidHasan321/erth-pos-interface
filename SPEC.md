...
**Shoulder slope** is recorded as one of six fixed values, picked from a plain text dropdown (no shape glyphs) — **RIGHT SHOULDER DOWN**, **RIGHT SHOULDER UP**, **RIGHT SHOULDER STRAIGHT**, **LEFT AND RIGHT SHOULDER DOWN**, **LEFT AND RIGHT SHOULDER UP**, **LEFT AND RIGHT SHOULDER STRAIGHT**. Shown unfilled, it must be picked before a measurement / add-garment record is saved (an absent value on a legacy row reads as unfilled and must be answered on edit). On the shop new-measurements form it sits side by side with collar position in one row.
